...
class VesselObjectTypeClass(object):
    ...
    async def build(self):
        # self.di_idx = await self.server.get_namespace_index("http://opcfoundation.org/UA/DI/")
        # self.ma_idx = await self.server.get_namespace_index("http://opcfoundation.org/UA/Machinery/")
        # self.functional_group_type = self.server.get_node(f"ns={self.di_idx};i=1005")
    
        vessel_object_type = await self.base_object_type.add_object_type(self.namespace, "VesselObjectType")

        # Metadaten
        vessel_meta_object = await vessel_object_type.add_object(self.namespace, "Meta", self.folder_object_type) # !!! FIXME !!! FunctionalGroupType + MachineryItemIdentificationType + SurfaceTechnologieSpecific Meta
        ...
        vessel_process_speed = await vessel_process_daten_object.add_object(self.namespace, "AgitatorSpeed")
        ...
        vessel_process_temperature = await vessel_process_daten_object.add_object(self.namespace, "Temperature")
